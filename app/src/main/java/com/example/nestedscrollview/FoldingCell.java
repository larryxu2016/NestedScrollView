...
//        Bitmap bitmapFromTitleView = measureViewAndGetBitmap(titleView, this.getMeasuredWidth());
        ...
        if (skipAnimation) {
            ...
            ArrayList<Integer> heights = calculateHeightsForAnimationParts(contentView.getHeight(), mAdditionalFlipsCount);
            // create list with animation parts for animation
            ArrayList<ImageView> foldingCellElements = prepareViewsForAnimation(heights, bitmapFromContentView);
...
//            this.mUnfolded = false;
     ...
     */
    protected ArrayList<ImageView> prepareViewsForAnimation(ArrayList<Integer> viewHeights, Bitmap contentViewBitmap) {
        if (viewHeights == null || viewHeights.isEmpty())
            throw new IllegalStateException("ViewHeights array must be not null and not empty");

        ArrayList<ImageView> partsList = new ArrayList<>();

        int partWidth = contentViewBitmap.getWidth();
        int yOffset = 0;
        for (int i = 0; i < viewHeights.size(); i++) {
            ...
            ImageView half = createImageViewFromBitmap(partBitmap);
            partsList.add(half);
     ...
     */
    protected ArrayList<Integer> calculateHeightsForAnimationParts(int contentViewHeight, int additionalFlipsCount) {
        ArrayList<Integer> partHeights = new ArrayList<>();
        // add two main parts - guarantee first flip
        partHeights.add(contentViewHeight/2);
        partHeights.add(contentViewHeight/2);
     ...
     */
    protected void startFoldAnimation(ArrayList<ImageView> foldingCellElements, ViewGroup foldingLayout,
                                      int part90degreeAnimationDuration, AnimationEndListener animationEndListener) {
        for (ImageView foldingCellElement : foldingCellElements)
        ...
        for (int i = 0; i < foldingCellElements.size(); i++) {
            ImageView cell = foldingCellElements.get(i);
            cell.setVisibility(VISIBLE);
            // if not last(top) element - animate whole view (i = 0)
            if (i != foldingCellElements.size() - 1) {
                cell.startAnimation(new FoldAnimation(FoldAnimation.FoldAnimationMode.FOLD_UP_UPPER, mCameraHeight, part90degreeAnimationDuration)
                        .withStartOffset(nextDelay)
                        .withInterpolator(new DecelerateInterpolator()));
                nextDelay = nextDelay + part90degreeAnimationDuration;
            }

            // not FIRST(BOTTOM) element  (i = 1)
            if (i != 0) {
                FoldAnimation foldAnimation = new FoldAnimation(FoldAnimation.FoldAnimationMode.FOLD_UP_LOWER, mCameraHeight, part90degreeAnimationDuration)
                        .withStartOffset(nextDelay)
                        .withInterpolator(new DecelerateInterpolator());
     ...
     * @param animationEndListener          animation end callback
     */
    protected void startUnfoldAnimation(ArrayList<ImageView> foldingCellElements, ViewGroup foldingLayout,
                                        int part90degreeAnimationDuration, AnimationEndListener animationEndListener) {
        int nextDelay = 0;
        for (int i = 0; i < foldingCellElements.size(); i++) {
            ImageView cell = foldingCellElements.get(i);
            ...
            if (i != foldingCellElements.size() - 1) {
                cell.startAnimation(new FoldAnimation(FoldAnimation.FoldAnimationMode.UNFOLD_UP, mCameraHeight, part90degreeAnimationDuration)
                        .withStartOffset(nextDelay)
                        .withInterpolator(new DecelerateInterpolator()));
                nextDelay = nextDelay + part90degreeAnimationDuration;
            }
